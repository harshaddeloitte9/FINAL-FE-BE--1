...
class Agent2:
    ...
    def check_data(self, df, col_types: dict, leakage_risk_cols: list | None = None) -> list[dict]:
        ...
        # SS1_P1.3_MACRO_MISSING — SS1/23 P1.3 requires documented macro inputs
        # (replaces R23 macro_variables_missing which covers the same condition)
        if not macro_found:
            flags.append({
                "rule_id": "SS1_P1.3_MACRO_MISSING",
                "source": "SS1/23",
                "principle": "Principle 1.3",
                "stage": "data",
                "severity": "high",
                "flag": (
                    "No macroeconomic variables detected in the dataset. "
                    "SS1/23 Principle 1.3 requires models used in regulatory decisions "
                    "to incorporate forward-looking economic inputs (e.g. GDP growth, "
                    "unemployment rate, interest rates) where material to model outcomes."
                ),
                "suggestion": (
                    "Add macroeconomic overlays to the feature set. "
                    "If macroeconomic inputs are not material for this portfolio, "
                    "document the justification formally in the model development log."
                ),
                "observed_value": None,
            })
    ...
    def check_training(
        ...
        # R11 cross_validation_used
        r = self._rule("cross_validation_used")
        if r and not t_cfg.get("use_cv", False):
            flags.append(self._make_flag(r, observed_value=False))

        # R12 smote_documented
        r = self._rule("smote_documented")
        if r and t_cfg.get("use_smote", False):
            flags.append(self._make_flag(
                r, observed_value=True,
                flag_override="SMOTE applied — ensure rationale and parameters are formally documented"
            ))

        # DEBUG: print training_info keys to diagnose SS1_P3.3_NO_CV false positives
        print(f"[DEBUG agent2 check_training] training_info = {ti}")

        # SS1_P3.3_NO_CV — high severity when CV not run at all
        if "cv_scores" not in ti and "cv_best_score" not in ti:
            flags.append({
                "rule_id": "SS1_P3.3_NO_CV",
                "source": "SS1/23",
                "principle": "Principle 3.3",
                "stage": "training",
                "severity": "high",
                "flag": (
                    "Cross-validation was not performed. SS1/23 P3.3 requires "
                    "backward-looking stability testing across data samples."
                ),
                "suggestion": "Enable K-Fold cross-validation in the training step.",
                "observed_value": None,
            })
        else:
        ...
        if imbalance_ratio is not None and imbalance_ratio > 10 and task_type == "binary":
            smote_applied = t_cfg.get("use_smote", False)
            class_weight_set = "class_weight" in (ti.get("best_params") or {})
            if not smote_applied and not class_weight_set:
                flags.append({
                    ...
                    "flag": (
                        ...
                        "mitigation applied. Neither SMOTE nor class weighting was used during "
                        "training. SS1/23 P3.2 requires imbalance to be addressed and its impact "
                        "on recall and precision to be documented."
                    ),
                    "suggestion": (
                        "Enable SMOTE in the sidebar at Step 6, or select a model with "
                        "class_weight support (Logistic Regression, Random Forest, Gradient "
                        "Boosting). Document the chosen approach and its impact on recall and PR-AUC."
                    ),
    ...
    def check_evaluation(
        ...
        # R17 explainability_performed
        r = self._rule("explainability_performed")
        if r and not explainability_done:
            flags.append(self._make_flag(r, observed_value=False))
        ...
        # SS1_P3.5_NO_SHAP — SHAP explainability required
        if not shap_available:
            flags.append({
                "rule_id": "SS1_P3.5_NO_SHAP",
                "source": "SS1/23",
                "principle": "Principle 3.5",
                "stage": "evaluation",
                "severity": "medium",
                "flag": (
                    "SHAP explainability analysis was not performed. "
