...
def train_model(
    ...
    2. Wrap in a Pipeline (with optional SMOTE).
    ...
    # ── SMOTE safety: k_neighbors must be < min class count ──
    if use_smote and task_type == "binary":
        min_class_count = int(y_train.value_counts().min())
        k = min(5, min_class_count - 1)
        if k < 1:
            use_smote = False   # not enough samples for SMOTE
            training_info["smote_skipped"] = f"Minority class has only {min_class_count} samples"
        else:
    ...
    training_info["smote_applied"] = use_smote
