import { n as PageHeader } from "./app-shell-DVyXktRn.js";
import { n as useDataset } from "./app-context-DEU1RUW-.js";
import { n as formUpload } from "./api-EJXRGsO6.js";
import { t as Button } from "./button-CRuuOnrV.js";
import { useRef, useState } from "react";
import { useNavigate } from "@tanstack/react-router";
import { jsx, jsxs } from "react/jsx-runtime";
import { ArrowRight, CheckCircle2, FileSpreadsheet, Sparkles, UploadCloud } from "lucide-react";
//#region src/routes/data-upload.tsx?tsr-split=component
function DataUpload() {
	const [progress, setProgress] = useState(0);
	const [file, setFile] = useState(null);
	const { setUploadResult, profile } = useDataset();
	const navigate = useNavigate();
	const inputRef = useRef(null);
	const uploadFile = async (f) => {
		if (!f) return;
		setFile({
			name: f.name,
			size: `${(f.size / (1024 * 1024)).toFixed(1)} MB`
		});
		setProgress(0);
		const t = setInterval(() => setProgress((p) => Math.min(90, p + 6)), 120);
		try {
			const form = new FormData();
			form.append("file", f);
			console.log("DataUpload: sending POST to /data/upload", {
				filename: f.name,
				size: f.size
			});
			const profile = await formUpload("/data/upload", form);
			console.log("DataUpload: received profile", profile);
			setProgress(100);
			setUploadResult(f, profile);
			if (profile && profile.shape) {
				const rows = profile.shape?.[0];
				setFile((prev) => prev ? {
					...prev,
					rows
				} : prev);
			}
		} catch (err) {
			setProgress(0);
			console.error("DataUpload: upload failed", err);
		} finally {
			clearInterval(t);
		}
	};
	return /* @__PURE__ */ jsxs("div", {
		className: "space-y-8",
		children: [
			/* @__PURE__ */ jsx(PageHeader, {
				title: "Data Upload",
				description: "Bring in loan tape, exposure, or scoring datasets. CSV and XLSX up to 5 GB."
			}),
			/* @__PURE__ */ jsxs("div", {
				className: "grid grid-cols-1 gap-6 lg:grid-cols-3",
				children: [/* @__PURE__ */ jsxs("div", {
					className: "lg:col-span-2 group relative cursor-pointer rounded-2xl border-2 border-dashed border-border bg-card p-10 text-center transition-colors hover:border-primary/60 hover:bg-primary/5",
					onClick: () => inputRef.current?.click(),
					onDragOver: (e) => e.preventDefault(),
					onDrop: (e) => {
						e.preventDefault();
						const f = e.dataTransfer.files[0];
						if (f) uploadFile(f);
					},
					children: [
						/* @__PURE__ */ jsx("input", {
							ref: inputRef,
							type: "file",
							accept: ".csv,.xlsx",
							className: "hidden",
							onChange: (e) => uploadFile(e.target.files?.[0] ?? null)
						}),
						/* @__PURE__ */ jsx("div", {
							className: "mx-auto flex h-16 w-16 items-center justify-center rounded-2xl gradient-primary shadow-elegant transition-transform group-hover:scale-105",
							children: /* @__PURE__ */ jsx(UploadCloud, { className: "h-8 w-8 text-primary-foreground" })
						}),
						/* @__PURE__ */ jsx("h3", {
							className: "mt-5 text-lg font-semibold",
							children: "Drop CSV or XLSX here"
						}),
						/* @__PURE__ */ jsx("p", {
							className: "mt-1 text-sm text-muted-foreground",
							children: "Or click to browse. We'll auto-detect schema and run profiling."
						}),
						/* @__PURE__ */ jsxs("div", {
							className: "mt-4 flex justify-center gap-2 text-[11px] text-muted-foreground",
							children: [
								/* @__PURE__ */ jsx("span", {
									className: "rounded-full border border-border bg-background px-2 py-0.5",
									children: "CSV"
								}),
								/* @__PURE__ */ jsx("span", {
									className: "rounded-full border border-border bg-background px-2 py-0.5",
									children: "XLSX"
								}),
								/* @__PURE__ */ jsx("span", {
									className: "rounded-full border border-border bg-background px-2 py-0.5",
									children: "Parquet (beta)"
								})
							]
						})
					]
				}), /* @__PURE__ */ jsxs("div", {
					className: "rounded-2xl border border-border bg-card p-6 shadow-elegant",
					children: [
						/* @__PURE__ */ jsxs("div", {
							className: "flex items-center gap-3",
							children: [/* @__PURE__ */ jsx("div", {
								className: "flex h-10 w-10 items-center justify-center rounded-lg bg-primary-soft",
								children: /* @__PURE__ */ jsx(Sparkles, { className: "h-5 w-5 text-primary" })
							}), /* @__PURE__ */ jsxs("div", { children: [/* @__PURE__ */ jsx("h3", {
								className: "text-sm font-semibold",
								children: "Synthetic dataset"
							}), /* @__PURE__ */ jsx("p", {
								className: "text-xs text-muted-foreground",
								children: "No data? Generate a realistic loan tape."
							})] })]
						}),
						/* @__PURE__ */ jsxs("ul", {
							className: "mt-4 space-y-2 text-xs text-muted-foreground",
							children: [
								/* @__PURE__ */ jsx("li", { children: "· 200k synthetic obligors, balanced default flag" }),
								/* @__PURE__ */ jsx("li", { children: "· IFRS 9 staging, macro overlay variables" }),
								/* @__PURE__ */ jsx("li", { children: "· Geographic + sector segmentation" })
							]
						}),
						/* @__PURE__ */ jsx("button", {
							onClick: async () => {
								setProgress(0);
								try {
									const t = setInterval(() => setProgress((p) => Math.min(90, p + 6)), 120);
									const form = new FormData();
									form.append("synthetic_samples", "200000");
									console.log("DataUpload: requesting synthetic dataset generation POST /data/upload", { synthetic_samples: 2e5 });
									const profile = await formUpload("/data/upload", form);
									console.log("DataUpload: synthetic profile received", profile);
									clearInterval(t);
									setProgress(100);
									setFile({
										name: "synthetic_portfolio.csv",
										size: "~200 MB",
										rows: profile?.shape?.[0] ?? void 0
									});
									setUploadResult(null, profile);
								} catch (err) {
									setProgress(0);
									console.error("DataUpload: synthetic generation failed", err);
								}
							},
							className: "mt-5 w-full rounded-lg border border-primary/30 bg-primary-soft px-3 py-2 text-sm font-medium text-foreground hover:bg-primary/20",
							children: "Generate dataset"
						})
					]
				})]
			}),
			file && /* @__PURE__ */ jsxs("div", {
				className: "rounded-2xl border border-border bg-card p-6 shadow-elegant",
				children: [
					/* @__PURE__ */ jsxs("div", {
						className: "flex items-start justify-between gap-4",
						children: [/* @__PURE__ */ jsxs("div", {
							className: "flex items-start gap-4",
							children: [/* @__PURE__ */ jsx("div", {
								className: "flex h-12 w-12 items-center justify-center rounded-lg border border-border bg-background",
								children: /* @__PURE__ */ jsx(FileSpreadsheet, { className: "h-6 w-6 text-primary" })
							}), /* @__PURE__ */ jsxs("div", { children: [/* @__PURE__ */ jsx("div", {
								className: "font-medium",
								children: file.name
							}), /* @__PURE__ */ jsxs("div", {
								className: "mt-1 flex flex-wrap gap-x-4 gap-y-1 text-xs text-muted-foreground",
								children: [
									/* @__PURE__ */ jsx("span", { children: file.size }),
									/* @__PURE__ */ jsx("span", { children: file.rows ? file.rows.toLocaleString() + " rows · 38 columns" : "— rows · 38 columns" }),
									/* @__PURE__ */ jsx("span", { children: "SHA-256 a4f…b921" })
								]
							})] })]
						}), progress >= 100 && /* @__PURE__ */ jsxs("span", {
							className: "inline-flex items-center gap-1.5 rounded-full bg-primary-soft px-3 py-1 text-xs font-medium text-foreground",
							children: [/* @__PURE__ */ jsx(CheckCircle2, { className: "h-4 w-4 text-primary" }), " Ready"]
						})]
					}),
					/* @__PURE__ */ jsx("div", {
						className: "mt-4 h-2 w-full overflow-hidden rounded-full bg-muted",
						children: /* @__PURE__ */ jsx("div", {
							className: "h-full gradient-primary transition-all",
							style: { width: `${progress}%` }
						})
					}),
					/* @__PURE__ */ jsxs("div", {
						className: "mt-1 flex justify-between text-[11px] text-muted-foreground",
						children: [/* @__PURE__ */ jsx("span", { children: progress < 100 ? "Uploading and validating schema…" : "Schema validated" }), /* @__PURE__ */ jsxs("span", { children: [progress, "%"] })]
					})
				]
			}),
			profile && progress >= 100 && /* @__PURE__ */ jsxs("div", {
				className: "grid gap-6 lg:grid-cols-[1fr_1.4fr]",
				children: [/* @__PURE__ */ jsxs("div", {
					className: "rounded-2xl border border-border bg-card p-6 shadow-elegant",
					children: [/* @__PURE__ */ jsx("h3", {
						className: "text-sm font-semibold",
						children: "Dataset summary"
					}), /* @__PURE__ */ jsxs("div", {
						className: "mt-4 space-y-3 text-sm text-foreground/90",
						children: [
							/* @__PURE__ */ jsxs("div", {
								className: "flex items-center justify-between border-b border-border/70 pb-3",
								children: [/* @__PURE__ */ jsx("span", {
									className: "text-xs text-muted-foreground",
									children: "Total rows"
								}), /* @__PURE__ */ jsx("span", {
									className: "font-semibold",
									children: profile.shape?.[0]?.toLocaleString() ?? "—"
								})]
							}),
							/* @__PURE__ */ jsxs("div", {
								className: "flex items-center justify-between border-b border-border/70 pb-3",
								children: [/* @__PURE__ */ jsx("span", {
									className: "text-xs text-muted-foreground",
									children: "Total columns"
								}), /* @__PURE__ */ jsx("span", {
									className: "font-semibold",
									children: profile.shape?.[1]?.toLocaleString() ?? "—"
								})]
							}),
							/* @__PURE__ */ jsxs("div", {
								className: "flex items-center justify-between border-b border-border/70 pb-3",
								children: [/* @__PURE__ */ jsx("span", {
									className: "text-xs text-muted-foreground",
									children: "Missing values"
								}), /* @__PURE__ */ jsx("span", {
									className: "font-semibold",
									children: profile.missing_cells?.toLocaleString() ?? "—"
								})]
							}),
							/* @__PURE__ */ jsxs("div", {
								className: "flex items-center justify-between",
								children: [/* @__PURE__ */ jsx("span", {
									className: "text-xs text-muted-foreground",
									children: "Duplicate rows"
								}), /* @__PURE__ */ jsx("span", {
									className: "font-semibold",
									children: profile.duplicate_rows?.toLocaleString() ?? "—"
								})]
							})
						]
					})]
				}), /* @__PURE__ */ jsxs("div", {
					className: "rounded-2xl border border-border bg-card p-6 shadow-elegant",
					children: [/* @__PURE__ */ jsx("div", {
						className: "flex items-center justify-between",
						children: /* @__PURE__ */ jsxs("div", { children: [/* @__PURE__ */ jsxs("h3", {
							className: "text-sm font-semibold",
							children: [
								"Preview first ",
								profile.data_preview?.length ?? 0,
								" rows"
							]
						}), /* @__PURE__ */ jsx("p", {
							className: "text-xs text-muted-foreground",
							children: "Loaded directly from backend response"
						})] })
					}), /* @__PURE__ */ jsx("div", {
						className: "mt-4 overflow-x-auto",
						children: /* @__PURE__ */ jsxs("table", {
							className: "min-w-full text-left text-sm",
							children: [/* @__PURE__ */ jsx("thead", { children: /* @__PURE__ */ jsx("tr", {
								className: "border-b border-border text-xs uppercase tracking-wider text-muted-foreground",
								children: (profile.columns ?? []).slice(0, 10).map((column) => /* @__PURE__ */ jsx("th", {
									className: "px-2 py-2",
									children: column
								}, column))
							}) }), /* @__PURE__ */ jsx("tbody", { children: (profile.data_preview ?? []).map((row, rowIndex) => /* @__PURE__ */ jsx("tr", {
								className: rowIndex % 2 === 0 ? "bg-background" : "bg-card",
								children: (profile.columns ?? []).slice(0, 10).map((column) => /* @__PURE__ */ jsx("td", {
									className: "whitespace-nowrap px-2 py-2 text-xs text-foreground/90",
									children: row[column] ?? ""
								}, `${rowIndex}-${column}`))
							}, rowIndex)) })]
						})
					})]
				})]
			}),
			file && progress >= 100 && /* @__PURE__ */ jsx("div", {
				className: "flex justify-end gap-3 pt-4",
				children: /* @__PURE__ */ jsxs(Button, {
					onClick: () => navigate({ to: "/profiling" }),
					className: "gap-2",
					children: ["Proceed to Profiling", /* @__PURE__ */ jsx(ArrowRight, { className: "h-4 w-4" })]
				})
			})
		]
	});
}
//#endregion
export { DataUpload as component };
