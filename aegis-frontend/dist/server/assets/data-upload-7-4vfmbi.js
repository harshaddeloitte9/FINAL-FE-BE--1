...
function DataUpload() {
	...
	const applyUploadResult = (uploadedFile, response) => {
		...
		const resolvedFile = uploadedFile ?? (typeof response?.csv_text === "string" ? new File([response.csv_text], datasetName.endsWith(".csv") ? datasetName : `${datasetName}.csv`, { type: "text/csv" }) : null);
		setUploadResult(resolvedFile, response);
