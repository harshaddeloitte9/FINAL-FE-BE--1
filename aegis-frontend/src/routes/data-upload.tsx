...
=======
  const testDatabaseConnection = async () => {
    ...
    try {
      ...
      if (dbType === "sqlite") {
        form.append("database", dbFilePath);
      } else {
        form.append("host", dbHost);
        form.append("port", dbPort);
        form.append("database", dbName);
        form.append("username", dbUsername);
        form.append("password", dbPassword);
      }
      console.log("DataUpload: testing DB connection", { dbType, dbHost, dbPort, dbName });
      await formUpload("/data/database/test", form);
    } catch (err) {
      console.error("DataUpload: DB test failed", err);
      setDbError(err instanceof Error ? err.message : "Failed to test database connection.");
    } finally {
  ...
  const loadDatabaseTables = async () => {
    ...
    try {
      ...
      console.log("DataUpload: loading DB tables", { dbType, dbHost, dbPort, dbName });
      const result = await formUpload<{ tables: string[] }>("/data/database/tables", form);
      setDbTables(result.tables ?? []);
      setSelectedTable(result.tables?.[0] ?? "");
    } catch (err) {
      console.error("DataUpload: load tables failed", err);
      setDbError(err instanceof Error ? err.message : "Failed to load database tables.");
    } finally {
  ...
  const fetchDatabaseTable = async () => {
    if (!selectedTable) {
      setDbError("Please select a table before fetching data.");
      return;
    }
    ...
    try {
      ...
      console.log("DataUpload: fetching DB table", { selectedTable });
      const profile = await formUpload("/data/database/fetch", form);
      console.log("DataUpload: received DB profile", profile);
      applyUploadResult(null, profile);
    } catch (err) {
      console.error("DataUpload: fetch table failed", err);
      setDbError(err instanceof Error ? err.message : "Failed to fetch database table.");
    } finally {
      setIsFetchingDb(false);
    }
  };

>>>>>>> 8f0d9100 (Add database connection upload method and SQLite support)
  return (
        ...
        <div className="rounded-xl border border-border bg-card p-6 shadow-elegant">
          <div className="flex items-start gap-3">
            ...
            <div>
              ...
              <div className="mt-2 text-sm text-muted-foreground">Connect to a relational database and fetch a table into the same credit risk pipeline.</div>
            </div>
          </div>
          ...
          <div className="mt-6 grid gap-3 sm:grid-cols-3">
            <button
              ...
              className="inline-flex items-center justify-center rounded-lg border border-primary/30 bg-primary-soft px-4 py-2 text-sm font-medium text-foreground hover:bg-primary/20 disabled:cursor-not-allowed disabled:opacity-60"
            >
            ...
            <button
              ...
              className="inline-flex items-center justify-center rounded-lg border border-border bg-background px-4 py-2 text-sm font-medium text-foreground hover:border-primary/40 hover:bg-primary-soft disabled:cursor-not-allowed disabled:opacity-60"
            >
              {isLoadingTables ? "Loading…" : "Load Tables"}
            </button>
            <button
              type="button"
              onClick={fetchDatabaseTable}
              disabled={isFetchingDb || !selectedTable}
              className="inline-flex items-center justify-center rounded-lg border border-primary/30 bg-primary-soft px-4 py-2 text-sm font-medium text-foreground hover:bg-primary/20 disabled:cursor-not-allowed disabled:opacity-60"
            >
              {isFetchingDb ? "Fetching…" : "Fetch Data"}
            </button>
          </div>
